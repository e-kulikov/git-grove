...
- `setup --agent <claude|codex|copilot> [--worktree NAME]` — write a project-local hook, scoped to one worktree, that denies any Edit/Write/Bash tool call (and, for `codex` specifically, its `apply_patch` tool) whose target resolves under `.bare` or the root `.git` file. The target is the worktree containing the current directory, or — anywhere else in the grove — the worktree holding the grove's recorded default branch; `--worktree` names one explicitly, relative to the grove root. `claude` and `copilot` share `.claude/settings.local.json`; run either once, not both. `codex` gets `.codex/config.toml`, with the hook inline as TOML. Running setup also records the agent in the grove's `grove.hookAgent` key, so `add` configures it in new worktrees too; a grove that records nothing is unaffected. Each agent's own trust and discovery rules still apply — `setup`'s own output names the exact next step.
- **Codex hook enforcement does not work in a grove today; do not rely on `setup --agent codex` for protection.** Codex resolves its project root by walking up for its own `project_root_markers` (default `.git`), and the grove root carries git-grove's own `.git` pointer file, so Codex treats the grove root as the project and never reads the worktree's `.codex/`. The config setup writes is correct but inert until git-grove's grove-root signature is renamed, which is a separate change that has not landed.
- **The Bash guard statically analyzes the command line; it cannot see inside an external interpreter.** It denies every construct it can fully account for (a directory change, `eval`/`exec`, subshells, command/process substitution, a variable in a path-looking argument, and more — including inside quoted content handed to another program). It cannot prove what a computed string given to a language it does not parse (`python3 -c '...'`, `perl -e '...'`) will actually do once that interpreter runs it. Closing that needs OS-level sandboxing, not another construct check.
